...
To search older mail safely, choose a **Search since** date and click **Preview matches**. Previewing sends nothing; it groups matches by sender display name and email address and shows the message count plus oldest/newest dates. After reviewing the summary, enter the Gmail destination and click **Forward previewed**, which requires a final confirmation. The dated preview checks both read and unread inbox messages and uses the same duplicate protection when forwarding. Original plain-text content is included inline so downstream Gmail and calendar workflows can read it, while the complete original remains attached.
