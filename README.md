...
To search older mail, choose a **Search since** date and click **Scan inbox now**. The dated scan checks both read and unread inbox messages, asks for confirmation before processing, applies only enabled rules, and uses the same duplicate protection. Original plain-text content is included inline in the forwarded email so downstream Gmail and calendar workflows can read it, while the complete original remains attached.
